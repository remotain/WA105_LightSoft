...
void dress_plot(ROOT::Fit::FitResult _res, TH1 * _h,  double _r_min, double _r_max){
	...
	_h->GetYaxis()->SetRangeUser(0.5, 100);
...
void fit_waveform_data(char * f_name){	
	...
	fitter.Config().ParSettings(2).Set("a_{1}"    , 1600 , 0.1);
	fitter.Config().ParSettings(3).Set("#tau_{1}" ,    6 , 0.1); fitter.Config().ParSettings(3).Fix();
	fitter.Config().ParSettings(4).Set("a_{2}"    ,  100 , 0.1);
	fitter.Config().ParSettings(5).Set("#tau_{2}" ,   57 , 0.1);
	fitter.Config().ParSettings(6).Set("a_{3}"    ,   19 , 0.1);
...
void fit_waveform_data_tail_only(char * f_name){	
...
void fit_waveform( int mode = 0, char * f_name = "$WLS_PLOT/simple_ana_run_00000211.root"){
	
	switch (mode) {
		case 0: // data
			fit_waveform_data(f_name); break;
		case 1: // mc 1
			fit_waveform_mc_1(); break;
		case 2: // tail only
			fit_waveform_data_tail_only(f_name); break;
		default: 
			fit_waveform_data(f_name); break;
	}
